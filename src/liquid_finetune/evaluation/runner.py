...
from liquid_finetune.evaluation.backend import HFBackend, InferenceBackend
...
def create_benchmarks_from_eval_config(
    ...
    *,
    ...
    if modality == "text":
        from liquid_finetune.evaluation.llm_config import (
            create_llm_benchmarks_from_config,
        )

        return create_llm_benchmarks_from_config(evals_dict, processor)
    if modality == "vlm":
        from liquid_finetune.evaluation.vlm_config import (
...
def load_eval_processor(model_ref: str, *, modality: str):
    from liquid_finetune.checkpointing.model_loading import _resolve_model_id
...
def create_hf_backend(
    ...
    *,
    ...
    from liquid_finetune.checkpointing.model_loading import load_model, load_vlm_model
...
def create_vllm_backend(model_ref: str, backend_config):
    from liquid_finetune.evaluation.backend import VLLMInProcessBackend
    from liquid_finetune.checkpointing.model_loading import _resolve_model_id
...
def _load_config(config):
    ...
    from liquid_finetune.config.parser import materialize_eval_config, parse_eval_config
