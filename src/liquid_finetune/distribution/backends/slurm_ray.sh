...
target = sys.argv[1]
_ = socket.AF_INET6 if ":" in target else socket.AF_INET
...
ray_slurm_stop_cluster() {
  ...
  # steps. Starting fresh `srun ray stop` commands can deadlock while those
  # blocking steps still occupy the allocation, so terminate the recorded
  # background srun PIDs directly and wait for Slurm to release them.
