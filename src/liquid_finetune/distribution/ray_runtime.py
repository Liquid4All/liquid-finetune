...
def patch_ray_rocm_torch_device_helpers() -> None:
    ...
    _get_rocm_worker_devices._liquid_rocm_patch = True  # type: ignore[attr-defined]

    def _get_rocm_worker_device():
        return _get_rocm_worker_devices()[0]

    _get_rocm_worker_device._liquid_rocm_patch = True  # type: ignore[attr-defined]
...
def get_requested_ray_address(ray_config: dict | None = None) -> str | None:
    ...
    for key in ("LIQUID_RAY_ADDRESS", "RAY_ADDRESS"):
        value = os.environ.get(key)
        if value:
            return value
...
def resolve_num_workers(
    ...
    *,
    ...
    for key in ("LIQUID_RAY_NUM_WORKERS", "LIQUID_NUM_WORKERS"):
    ...
    if connected_to_existing_cluster:
        ...
        if cluster_gpus < 1:
            raise ValueError(
                ...
                "Set LIQUID_RAY_NUM_WORKERS if cluster resources are still registering."
            )
        return cluster_gpus
...
def resolve_local_ray_num_cpus() -> int | None:
    ...
    raw = os.environ.get("LIQUID_RAY_NUM_CPUS")
    if raw:
        return int(raw)
