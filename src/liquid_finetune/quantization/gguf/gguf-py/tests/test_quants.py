...
def do_test(
    libggml_path: Path,
    quick: bool = False,
    user_type: GGMLQuantizationType | None = None,
):
    ...
    for qtype in (
        (GGMLQuantizationType.F16, *gguf.quants._type_traits.keys())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--libggml",
        type=Path,
        default=Path(__file__).parent.parent.parent / "build" / "bin" / "libggml.so",
    ...
    parser.add_argument(
        "--type", type=str, help="The quant type to test (all by default)"
    )
    ...
    do_test(
        args.libggml,
        args.quick,
        GGMLQuantizationType[args.type.upper()] if args.type is not None else None,
    )
