...
class TextModel(ModelBase):
    ...
    @staticmethod
    def _qwen_token_bytes_to_string(b):
    ...
    def _set_vocab_qwen(self):
        ...
        for token, rank in mergeable_ranks.items():
            vocab[self._qwen_token_bytes_to_string(token)] = rank
            if len(token) == 1:
                continue
            merged = self._qwen_bpe(mergeable_ranks, token, max_rank=rank)
            assert len(merged) == 2
            merges.append(" ".join(map(self._qwen_token_bytes_to_string, merged)))
    ...
    def _set_vocab_mistral(self):
        ...
        if self.is_mistral_format and local_template_file_path.is_file():
            ...
            template = (
                None  # MistralModel removed; community chat templates not supported
            )
        else:
            logger.info(
...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        if not is_mistral_format:
            ...
            logger.error("Mistral format is not supported in this pruned build")
            sys.exit(1)
