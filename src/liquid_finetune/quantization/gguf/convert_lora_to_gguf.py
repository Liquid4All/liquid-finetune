    ...
    with torch.inference_mode():
        ...
        class LoraModel(model_class):
            ...
            def __init__(
                ...
                self._q8_0l_tensors: set[str] = set()
                self._q8_0l16_tensors: set[str] = set()
            ...
            def tensor_force_quant(
                ...
                if new_name in self._q8_0l16_tensors:
                    return gguf.GGMLQuantizationType.Q8_0_L16
                if new_name in self._q8_0l_tensors:
                    return gguf.GGMLQuantizationType.Q8_0_L
            ...
            def set_gguf_parameters(self):
                logger.debug(
                    "GGUF KV: %s = %d", gguf.Keys.Adapter.LORA_ALPHA, self.lora_alpha
                )
                ...
                if invocation_string and not alora_invocation_tokens:
                    logger.debug(
                        "Tokenizing invocation_string -> alora_invocation_tokens"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            "Unable to load tokenizer from %s", base_model_path_or_id
                        )
                        raise
            ...
            def modify_tensors(
                ...
                for dest_name, dest_data in dest:
                    ...
                    # for lora_b, check if the rank is too small for Q8_0 (block_size=32)
                    # rank-16: use Q8_0_L16 (block_size=16)
                    # rank-8/24: use Q8_0_L (block_size=8)
                    if ftype == gguf.LlamaFileType.MOSTLY_Q8_0:
                        if lora_b.shape[-1] == 16:
                            self._q8_0l16_tensors.add(dest_name + ".lora_b")
                        elif lora_b.shape[-1] % 8 == 0 and lora_b.shape[-1] < 32:
                            self._q8_0l_tensors.add(dest_name + ".lora_b")
